...
| `author_profile/gate_artifact.md` | **The rendered-artifact gate (A1–A9).** Compile + grep the `.log` (overfull boxes, undefined refs, missing assets), then render + inspect every changed page for clipped/overflowing floats, reused-float refit, legibility, color-as-only-channel, caption terseness, and placement. Catches what a source-only pass cannot see. Run on every edit that touches a float, caption, or table. |
...
8. **Rendered artifact (`gate_artifact.md`)**: any edit that adds or changes a float, caption, or table also runs the artifact gate — compile, grep the `.log` for overfull boxes and undefined references, then render every changed page to an image and inspect its floats for clipping/overflow, reused-float refit, legibility, color-as-only-channel, caption terseness, and placement. A source-only pass is half a gate; a figure or table is not audited until it is rendered and looked at.

**Process**: After writing, (a) run the grep gate in `gate_mechanical.md` Part C and fix every hit, then (b) read the changed text line by line for the items above. Report a summary table of violations found and fixed (category, count), INCLUDING the grep counts (em-dashes, flourishes) — not just "audited". Never claim the audit passed on a mental pass alone. Then (c) run the **independent adversarial red-team** (`red_team_protocol.md`): a reviewer that did NOT write the text re-runs `gate_mechanical.md` and applies `author_profile/gate_semantic.md` (define-before-use, followability, thesis-tie, lexical + decomposition consistency, non-duplication, mappability, honest positioning, and the closure gate) with a fresh-reader lens, returning a findings list, not a yes/no. Then (d), for any edit that adds or changes a float, caption, or table, run the **artifact gate** (`author_profile/gate_artifact.md`): compile, grep the `.log` for overfull boxes and undefined references, and render every changed page to an image and inspect its floats; hand the red-team the rendered images, not only the `.tex`, so it can catch layout and float defects a source read cannot. Only text that survives (a) + (b) + (c) + (d), with the grep output and rendered-page evidence pasted, is presented or committed. Per `gate_semantic.md`'s **closure gate** (S31), iterate the red-team after every substantive change until a final closure reviewer returns zero CRITICAL/MAJOR; never defer residual items as "done."
